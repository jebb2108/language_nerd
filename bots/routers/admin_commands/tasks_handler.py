...
from config import ADMIN_ID # noqa

from middlewares.resources_middleware import ResourcesMiddleware # noqa
from ai_handler import ( # noqa
    generate_weekly_reports,# noqa
    cleanup_old_reports, # noqa
    send_pending_reports,  # noqa
) # noqa

from config import TELEGRAM_API_SEMAPHORE, TELEGRAM_MIN_DELAY_BETWEEN_REQUESTS, LOG_CONFIG # noqa
from routers.commands.weekly_message_commands import send_user_report # noqa
from keyboards.inline_keyboards import begin_weekly_quiz_keyboard # noqa
from bots.utils.database import Database
...
async def send_reports(
    ...
    if message.from_user.id == int(ADMIN_ID):
        await send_pending_reports(message.bot, database)
        return
    return message.reply(text='You don`t have a permission')
